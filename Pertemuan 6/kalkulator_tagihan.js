...
// Langkah 2: Seleksi Elemen di JavaScript
...
// Langkah 4 Event Input (Validasi Real-time)
...
// LATIHAN 1 (DOM STYLING)
// Untuk tombol maintenance
const btnMaintenance = document.getElementById("btnMaintenance");
const cardMesin = document.getElementById("cardMesin");

// event click
btnMaintenance.addEventListener("click", function () {
  cardMesin.classList.add("bg-light");
...
// TUGAS MINI : KALKULATOR TAGIHAN LISTRIK
...
function hitungListrik() {
  ...
  if (!daya || !jam) {
    hasilKwh.innerText = "0";
    hasilBiaya.innerText = "Rp 0";
    return;
  }
  ...
  hasilKwh.innerText = kwh.toFixed(2);
  hasilBiaya.innerText = "Rp " + biaya.toLocaleString();
  updateStatus(kwh);
}
...
// STATUS PEMAKAIAN
const statusListrik = document.getElementById("statusListrik");

function updateStatus(kwh) {
  if (!statusListrik) return; // biar tidak error

  if (kwh > 10) {
    statusListrik.innerText = "Status: Pemakaian Tinggi";
  } else if (kwh > 5) {
    statusListrik.innerText = "Status: Pemakaian Sedang";
  } else {
    statusListrik.innerText = "Status: Hemat";
  }
}
