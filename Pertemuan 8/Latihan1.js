...
// Latihan 1: POST Data Karyawan Baru
...
btnTambah.addEventListener("click", function () {
  ...
  const karyawanBaru = {
    ...
    email: "nanikwijanti@majujaya.com",
    phone: "081234567890",
    company: { name: "PT Maju Jaya" },
  ...
  fetch(API_URL, {
    ...
    .then(function (response) {
      if (!response.ok) {
        throw new Error(`POST gagal: ${response.status}`);
      }
      return response.json();
    })
    .then(function (data) {
      console.log("KARYAWAN BARU DITERIMA SERVER:", data);
      alert(`Sukses! ID: ${data.id}\nName: ${data.name}`);

      // Auto reload data untuk lihat hasil
      document.getElementById("btnLoad").click();
    })
    .catch(function (error) {
      console.error("POST ERROR:", error);
      alert("Gagal tambah karyawan: " + error.message);
    });
});
